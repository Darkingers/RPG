...
namespace RPG
{
    ...
    class Effect:ScriptObject
    {
        ...
        public Effect()
        {
            Copy(0, 0, null, new List<Tag>(), new List<Tag>(), Number.Current, Modifier.Decrease,new Cooldown(new ScriptObject(),0,0,null));
        }
        public Effect(ScriptObject scriptobject,double flat,double percent,Stat target,List<Tag> sender,List<Tag> reciever,Number target_number,Modifier intention,Cooldown duration) : base(scriptobject)
        {
            Copy(flat, percent, target, sender, reciever, target_number, intention, duration);
        }
        public Effect(Effect cloned)
        {
            Copy(cloned);
        }
        public bool Copy(Effect copied)
        {
        ...
        public double Get_Value()
        {
            switch (Intention)
            {
                case Modifier.Decrease:return Flat * ((100+Percent)/100) * -1;
                case Modifier.Increase:return Flat * ((100+Percent)/100);
                default:return 0;
            }
        }
        ...
        public override bool Set_Variable(string name, object value)
        {
            switch (name)
            {
                ...
                case "Sender_Tags":return Set_Sender_Tags(Converter.Convert_Array<Tag>(value));
                case "Reciever_Tags": return Set_Reciever_Tags(Converter.Convert_Array<Tag>(value));
        ...
        {
            if (((Effect)compared).Equals(this))
            {
                return 0;
            }
            else
