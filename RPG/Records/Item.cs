...
namespace RPG
{
    class Item:Record
    {
        ...
        {
            string text = Get_Identifier() + Environment.NewLine + Description+Environment.NewLine+"Stats/";
            foreach(Item_Stat stat in Stats)
            {
            ...
            if (Skills.Count > 0)
            {
                text += Environment.NewLine + "Granted skills/";

                foreach (Skill skill in Skills)
                {
                    text += Environment.NewLine + skill.Get_Identifier();
